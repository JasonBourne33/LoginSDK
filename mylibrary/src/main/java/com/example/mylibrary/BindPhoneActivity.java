...
public class BindPhoneActivity extends AppCompatActivity {
    ...
    @OnClick({R2.id.tv_sms, R2.id.btn_save, R2.id.rl_back})
    public void onViewClicked(View view) {
        ...
        if (i == R.id.tv_sms) {
            ...
            if (etEnterPhone.getText().toString().equals("")) {
                ...
                String strJson = bindPhoneToJson();
                Log.e(TAG, "onViewClicked: strJson=== " + strJson);
                String finalUrl = UrlUtil.getQuestUrl(bindPhoneUrl, strJson);
                bindPhoneRequest(finalUrl);
            }
        } else if (i == R2.id.rl_back) {
            finish();
        }
    }
...
//                    Response response = client.newCall(request).execute();
    ...
    public String bindPhoneUrl = "http://apisdk.98gam.com/v1/member/bind-info";

    private String bindPhoneToJson() {
        JSONObject jsonObj = new JSONObject();
        try {
            jsonObj.put("tel", etEnterPhone.getText().toString());
            jsonObj.put("code", etSms.getText().toString());
            jsonObj.put("scenario", "bindtel");
        } catch (JSONException e) {
            e.printStackTrace();
        }
        return jsonObj.toString();
    ...
    private void bindPhoneRequest(String url) {
